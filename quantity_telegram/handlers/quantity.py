...
async def minimum(message: Message):
    stocks = await message.bot.get_stocks()
    if not stocks:
...
async def show_minimum(callback: CallbackQuery):
    ...
    try:
        ...
        stocks = await callback.bot.get_stocks()
        stock_name = next((s["name"] for s in stocks if s["id"] == stock_id), "Склад")

        items = await callback.bot.get_items()
        if not items:
        ...
        quantities = await callback.bot.get_current_quantity(
        ...
        low = [
            {
                "Товар": i["name"],
                "Остаток": quantities.get(i["id"], {}).get(stock_id, 0),
                "Мин.": i.get("min_quantity", 0),
            }
            for i in items
        ...
        if not low:
            return await loading_msg.edit_text(f"На складе «{stock_name}» товары в норме!")
        ...
        await callback.bot.send_document(
            ...
            document=file,
            caption=(
                f"Минимальные остатки\n"
                f"Склад: {stock_name}\n"
                f"Товаров ниже минимума: {len(low)}"
            ),
            parse_mode="Markdown"
        )
...
# ====== ПОИСК ПО ТЕКСТУ (оригинальный функционал) ======
...
async def search(message: Message):
    ...
    items = await message.bot.get_items()
    ...
    stocks = await message.bot.get_stocks()
    quantities = await message.bot.get_current_quantity(
        [i["id"] for i in found],
